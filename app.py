...
class SistemaAcademicoUFF:
    ...
    def login(self, username, password):
        ...
        try:
            ...
            login_page = self.session.get(ADMIN_ACAD_URL)
...
def criar_aba_acumulado(wb, dados_por_periodo):
    ...
    """
    ws = wb.create_sheet(title="Acumulado")
    ...
    titulo = f"Acumulado de {periodos[0]} a {periodos[-1]}"
    
    # === LINHA 1: Título principal ===
    ws.merge_cells('A1:CE1')
    cell = ws['A1']
    cell.value = "QUÍMICA IQ - LEVANTAMENTO MATRÍCULAS SISU"
    cell.font = Font(bold=True, color="FFFFFF", size=12)
    cell.fill = PatternFill(start_color="1F4E79", end_color="1F4E79", fill_type="solid")
    ...
    # === LINHA 2: Cabeçalho com períodos ===
    # Esta linha terá os períodos em grupos de 3 colunas (AC, AA, GERAL)
    col_atual = 13  # Começa na coluna N
    
    for periodo in periodos:
        # Célula do período
        ws.merge_cells(start_row=2, start_column=col_atual, end_row=2, end_column=col_atual+2)
        cell = ws.cell(row=2, column=col_atual, value=periodo)
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal='center', vertical='center')
        cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    ...
    # Adiciona "RESUMO PARA GRÁFICOS" no final
    ws.merge_cells(start_row=2, start_column=col_atual, end_row=2, end_column=col_atual+4)
    cell = ws.cell(row=2, column=col_atual, value="RESUMO PARA GRÁFICOS")
    cell.font = Font(bold=True)
    cell.alignment = Alignment(horizontal='center', vertical='center')
    cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                        top=Side(style='thin'), bottom=Side(style='thin'))
    
    # === LINHA 3: Cabeçalho de modalidades para cada período ===
    ws['A3'] = "Modalidade"
    ws['A3'].font = Font(bold=True)
    ws['A3'].alignment = Alignment(horizontal='center', vertical='center')
    ws['A3'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    # Cabeçalho principal (similar à aba individual)
    ws.merge_cells('B3:E3')
    ws['B3'] = "AMPLA CONCORRÊNCIA"
    ...
    ws['B3'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    ws.merge_cells('F3:I3')
    ws['F3'] = "AÇÕES AFIRMATIVAS"
    ws['F3'].font = Font(bold=True, color="FFFFFF")
    ws['F3'].fill = PatternFill(start_color="548235", end_color="548235", fill_type="solid")
    ws['F3'].alignment = Alignment(horizontal='center', vertical='center')
    ws['F3'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    ws.merge_cells('J3:K3')
    ws['J3'] = "TOTAIS"
    ws['J3'].font = Font(bold=True, color="FFFFFF")
    ws['J3'].fill = PatternFill(start_color="C65911", end_color="C65911", fill_type="solid")
    ws['J3'].alignment = Alignment(horizontal='center', vertical='center')
    ws['J3'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    # Para cada período, adiciona AC, AA, GERAL
    col_atual = 13
    for periodo in periodos:
        for i, modalidade in enumerate(["(AC)", "(AA)", "GERAL"]):
            cell = ws.cell(row=3, column=col_atual + i, value=modalidade)
            cell.font = Font(bold=True)
            cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                                top=Side(style='thin'), bottom=Side(style='thin'))
    
    # Para a seção de resumo
    resumo_cols = ["Período", "Ingressantes", "Cancelamentos", "Matrículas Ativas", "Formados", "% Evasão"]
    for i, tit in enumerate(resumo_cols):
        cell = ws.cell(row=3, column=col_atual + i, value=tit)
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal='center', vertical='center')
        cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    # === LINHA 4: Cabeçalho de cursos ===
    ws['A4'] = "Curso"
    ws['A4'].font = Font(bold=True, color="FFFFFF")
    ws['A4'].fill = PatternFill(start_color="1F4E79", end_color="1F4E79", fill_type="solid")
    ws['A4'].alignment = Alignment(horizontal='center', vertical='center')
    ws['A4'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    cursos = ["Licenciatura Química", "Bacharel Química", "Bacharel Q Industrial", 
              "TOTAL", "Licenciatura Química", "Bacharel Química", 
              "Bacharel Q Industrial", "TOTAL", "TOTAL", "%"]
    
    cores = ["1F4E79"] * 4 + ["548235"] * 4 + ["C65911"] * 2
    
    for i, (curso, cor) in enumerate(zip(cursos, cores), 1):
        col = i + 1 if i <= 10 else i + 1  # Ajuste para pular coluna A
        if i > 10:
            col = i + 1  # Corrige offset
        cell = ws.cell(row=4, column=col, value=curso)
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color=cor, end_color=cor, fill_type="solid")
        cell.alignment = Alignment(horizontal='center', vertical='center')
        cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
    ...
    # === LINHA 5: Subtítulos ===
    ws['A5'] = " "
    ws['A5'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    subtitulos = [" ", " ", " ", "(AC)", " ", " ", " ", "(AA)", "GERAL", "GERAL"]
    for i, subtitulo in enumerate(subtitulos, 1):
        cell = ws.cell(row=5, column=i+1, value=subtitulo)
        cell.alignment = Alignment(horizontal='center', vertical='center')
        cell.border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    # Agora preenche os dados
    # Como esta é uma versão simplificada, vamos preencher com fórmulas de referência
    # Em uma versão real, você processaria todos os dados aqui
    
    # Linha 6: Total de Ingressantes
    ws['A6'] = "Total de Ingressantes"
    ws['A6'].alignment = Alignment(horizontal='left', vertical='center')
    ws['A6'].border = Border(left=Side(style='thin'), right=Side(style='thin'), 
                            top=Side(style='thin'), bottom=Side(style='thin'))
    
    # Para cada período, adiciona referências às abas correspondentes
    col_atual = 13
    for periodo in periodos:
        nome_aba = str(periodo).replace("/", ".").replace("°", "")
        # AC
        ws.cell(row=6, column=col_atual).value = f"='{nome_aba}'!E5"
        # AA
        ws.cell(row=6, column=col_atual+1).value = f"='{nome_aba}'!I5"
        # GERAL
        ws.cell(row=6, column=col_atual+2).value = f"='{nome_aba}'!J5"
        col_atual += 3
    
    # Preenche a seção de resumo para gráficos
    resumo_col = col_atual
    for i, periodo in enumerate(periodos):
        nome_aba = str(periodo).replace("/", ".").replace("°", "")
        ws.cell(row=6+i, column=resumo_col).value = periodo
        ws.cell(row=6+i, column=resumo_col+1).value = f"='{nome_aba}'!J5"
        ws.cell(row=6+i, column=resumo_col+2).value = f"='{nome_aba}'!J13"
        ws.cell(row=6+i, column=resumo_col+3).value = f"='{nome_aba}'!J16"
        ws.cell(row=6+i, column=resumo_col+4).value = f"='{nome_aba}'!J19"
        ws.cell(row=6+i, column=resumo_col+5).value = f"=1-({get_column_letter(resumo_col+3)}{6+i}+{get_column_letter(resumo_col+4)}{6+i})/{get_column_letter(resumo_col+1)}{6+i}"
        ws.cell(row=6+i, column=resumo_col+5).number_format = '0.00%'
...
def gerar_planilha_completa(dados_por_periodo):
    ...
    # 4. Cria aba Modelo (opcional)
    ws_modelo = wb.create_sheet(title="Modelo")
    # Copia a estrutura de uma aba existente para servir como modelo
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        **Instruções:**
        ...
        2. Nomeie os arquivos no formato: `CURSO_PERIODO.xlsx`
        ...
        **Exemplos de nomes:**
        ...
        if st.button("Gerar Planilha de Exemplo (baseada no modelo fornecido)"):
